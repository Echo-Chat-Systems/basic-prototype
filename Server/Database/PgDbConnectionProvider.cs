...
public class PgDbConnectionProvider : IDbConnectionProvider
{
    ...
    public PgDbConnectionProvider(Config allConfig)
    {
        ...
        _connectionString =
            ...
            $"Username={config.Credentials["Main"].Username};" +
            $"Password={config.Credentials["Main"].Password};"
#if DEBUG
            + $"Include Error Detail=true;"
#endif
            ;
        
        // Populate _source if not present
        if (_source == null)
        {
            _source = new NpgsqlDataSourceBuilder(_connectionString)
                .EnableDynamicJson()
                .Build();
    ...
    public async Task<T> Get<T>() where T : DbConnection
    {
        ...
        if (typeof(T) != typeof(NpgsqlConnection))
            throw new InvalidOperationException(
                $"{nameof(PgDbConnectionProvider)} only supports {nameof(NpgsqlConnection)}");

        // Create and return a new NpgsqlConnection
        NpgsqlConnection con = _source!.CreateConnection();
        await con.OpenAsync();
